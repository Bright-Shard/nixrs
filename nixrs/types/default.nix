...
let
  ...
  types = {
    # Extra settings to pass to nixrs' `rustc` function.
    compilation-setting = import ./compilation-setting.nix nixrs;
    ...
    # A semantic version. The type itself only stores the major, minor, and patch
    # version numbers, but it has a `from-string` method that allows converting
    # from traditional semantic version strings: https://semver.org
    semantic-version = import ./semantic-version.nix nixrs;
    # A dependency version specifier. The type itself only stores the minimum
    # and maximum supported versions of a crate, but it has a `from-string` method
    # that allows converting from Cargo version specifiers:
